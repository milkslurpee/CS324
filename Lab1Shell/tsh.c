...
void eval(char *cmdline)
{
        ...
        if (builtin_cmd(argv) == 0){
                ...
                int commands[MAXARGS], stdin_redir[MAXARGS], stdout_redir[MAXARGS];
                int numOfCommands = parseargs(argv, commands, stdin_redir, stdout_redir);
                int numOfPipes = numOfCommands - 1;

                int pipeFDs[numOfPipes][2];
                int childPids[numOfCommands];
                pid_t pgid = -1;

                for (int i = 0; i < numOfPipes; i++) {
                        if(pipe(pipeFDs[i]) == -1) return;
                }
                ...
                for (int i = 0; i < numOfCommands; i++) {
                        ...
                        if (pid == 0) {
                                ...
                                if (stdin_redir[i] >= 0) {
                                        ...
                                        int fDescriptor = open(filename, O_RDONLY);
                                        if (fDescriptor == -1) exit(1);
                                        dup2(fDescriptor, STDIN_FILENO);
                                        close(fDescriptor);
                                }
                                else if (i > 0) {
                                ...
                                if (stdout_redir[i] >= 0) {
                                        ...
                                        int fDescriptor = open(filename, O_WRONLY | O_CREAT | O_TRUNC, 0600);
                                        if (fDescriptor == -1) exit(1);
                                ...
                                for (int fDescriptor = 3; fDescriptor < MAXLINE; fDescriptor++) {
                                        close(fDescriptor);
                                }

                                execve(argv[commands[i]], &argv[commands[i]], environ);
                                exit(1);
